...
class SlicingSelector(Selector):
    ...
    def __init__(self, list_of_slicing_functions = None):
        super().__init__()
        self.sfs = self.create_list_of_slicing_functions(list_of_slicing_functions)

    def select(self, dataset: pd.DataFrame) -> pd.DataFrame:
        self.create_slicing_functions(dataset)
        return dataset
    
    def create_list_of_slicing_functions(self, list_of_slicing_functions: Union[List[Callable], None]):
